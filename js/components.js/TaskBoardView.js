...
export default class TaskBoardView {
  constructor(boardId, store) {
    ...
    this.columns = {
      [STATUS.PENDING]: this.container.querySelector('.task__column--pending'),
      [STATUS.IN_PROGRESS]: this.container.querySelector('.task__column--inprogress'),
      [STATUS.COMPLETED]: this.container.querySelector('.task__column--completed'),
    };
  ...
  render() {
    ...
    [STATUS.PENDING, STATUS.IN_PROGRESS, STATUS.COMPLETED].forEach(status => {
      const tasks = this.store.getTasksByStatus(status);
      tasks.forEach(task => {
        const taskHTML = new TaskView(task).render();
        this.columns[status].insertAdjacentHTML('beforeend', taskHTML);
      });
    });
  }
}
